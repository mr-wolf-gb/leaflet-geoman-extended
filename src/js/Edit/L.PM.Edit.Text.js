...
Edit.Text = Edit.extend({
  ...
  _setupDoubleClickHandler() {
    // Add double-click handler to show modal
    this._layer.on('dblclick', (e) => {
      L.DomEvent.stop(e);
      this._showEditModal();
    });
  },
  ...
  _showEditModal() {
    const currentText = this.isRichText
      ? this.textArea.innerHTML
      : this.getText();
    // Get background color from options first (stored as hex), fallback to computed style, then default
    const currentBgColor =
      this.options.backgroundColor ||
      this._rgbToHex(this.textArea?.style.backgroundColor) ||
      '#ffffff';
    const currentFontSize = this.options.fontSize || 14;
  ...
  _updateFromModal(modalResult) {
    // Update content based on whether it's rich text or plain text
    if (this.isRichText) {
      this.textArea.innerHTML = modalResult.text;
    } else {
      this.setText(modalResult.text);
    }

    // Apply styles
    if (modalResult.backgroundColor) {
      this.textArea.style.backgroundColor = modalResult.backgroundColor;
      this.options.backgroundColor = modalResult.backgroundColor;
    }
    if (modalResult.fontSize) {
      this.textArea.style.fontSize = `${modalResult.fontSize}px`;
      this.options.fontSize = modalResult.fontSize;
      if (this._autoResize) {
        this._autoResize();
      }
    }
  ...
  _autoResize() {
    ...
    this.textArea.style.height = 'fit-content'; //`${height}px`;
    this.textArea.style.width = 'fit-content'; //`${width}px`;
