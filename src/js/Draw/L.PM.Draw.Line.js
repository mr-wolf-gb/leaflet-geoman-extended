...
Draw.Line = Draw.extend({
  ...
  _syncHintLine() {
    ...
    if (polyPoints.length > 0) {
      ...
      this._updateLineMeasurementsOnMove();
    }
  },
  _updateLineMeasurementsOnMove() {
    if (
    ...
    // Get current tooltip text based on shape type
    const { length } = this._layer.getLatLngs().flat();
    let text = '';

    if (this._shape === 'Polygon') {
      // Polygon-specific text
      if (length <= 2) {
    ...
    // Create a temporary layer with cursor position included for measurement
    const cursorPosition = this._hintMarker.getLatLng();
    const existingLatLngs = this._layer.getLatLngs();
    const tempLatLngs = [...existingLatLngs, cursorPosition];

    // Create temporary layer for measurement calculation
    let tempLayer;
    if (this._shape === 'Polygon') {
      tempLayer = L.polygon(tempLatLngs);
    } else {
      tempLayer = L.polyline(tempLatLngs);
    }

    // Calculate measurements with cursor position included
    const measurements = L.PM.Utils.getMeasurements(
      tempLayer,
      this._map,
      this.options.measurements.displayFormat || 'metric'
    );
  ...
  _finishShape() {
    ...
    if (this.options.measurements && this.options.measurements.measurement) {
      this._updateLayerMeasurements(polylineLayer);
      // Bind hover tooltip
      if (this.options.measurements.showTooltipOnHover !== false) {
        this._bindMeasurementTooltip(polylineLayer);
      }
    }
