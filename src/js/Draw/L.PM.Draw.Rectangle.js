...
Draw.Rectangle = Draw.extend({
  ...
  _syncRectangleSize() {
    ...
    this._updateRectangleMeasurements();
  },
  _updateRectangleMeasurements() {
    if (
    ...
    if (measurements) {
      ...
      if (measurementText) {
        const currentText = getTranslation('tooltips.finishRect');
  ...
  _finishShape(e) {
    // Check if starting marker has been placed yet
    if (
      !this._startMarker ||
      !this._layerGroup ||
      !this._layerGroup.hasLayer(this._layer)
    ) {
      // No starting marker placed yet, just cancel drawing
      this.disable();
      return;
    }
    ...
    if (this.options.measurements && this.options.measurements.measurement) {
      this._updateLayerMeasurements(rectangleLayer);
      // Bind hover tooltip
      if (this.options.measurements.showTooltipOnHover !== false) {
        this._bindMeasurementTooltip(rectangleLayer);
      }
    }
